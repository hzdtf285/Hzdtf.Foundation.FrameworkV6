...
namespace Hzdtf.AUC.AspNet
{
        ...
        where UserT : BasicUserInfo<IdT>
    {
        ...
        public IdentityJwtAuth(IUserVali<IdT, UserT> userVali = null, IAuthUserData<IdT, UserT> authUserData = null, IHttpContextAccessor httpContext = null)
        {
        ...
        private string BuilderToken(IList<Claim> claims)
        {
            ...
            var overTime = currTime.AddMinutes(expires);

            claims.Add(JwtRegisteredClaimNames.Nbf, $"{new DateTimeOffset(currTime).ToUnixTimeSeconds()}");
            claims.Add(JwtRegisteredClaimNames.Exp, $"{new DateTimeOffset(overTime).ToUnixTimeSeconds()}");
