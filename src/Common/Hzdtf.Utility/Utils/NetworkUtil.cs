...
namespace Hzdtf.Utility.Utils
{
    ...
    public static class NetworkUtil
    {
        ...
        /// 无符号整型转换为IP字符串
        /// </summary>
        /// <param name="ipUint">无符号整型IP</param>
        /// <returns>IP字符串</returns>
        public static string UIntToIp(uint ipUint)
        {
            if (ipUint == 0)
            {
                return null;
            }
            var netInt = (uint)IPAddress.HostToNetworkOrder((Int32)ipUint);
            var ipaddr = new IPAddress((long)netInt);
