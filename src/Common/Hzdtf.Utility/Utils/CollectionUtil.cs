...
namespace Hzdtf.Utility.Utils
{
    ...
    public static class CollectionUtil
    {
        ...
        /// 将源字典转换为指定类型的目标字典
        /// </summary>
        /// <typeparam name="SourceKeyT">源键类型</typeparam>
        /// <typeparam name="SourceValueT">源值类型</typeparam>
        /// <typeparam name="TargetKeyT">目标键类型</typeparam>
        /// <typeparam name="TargetValueT">目标值类型</typeparam>
        /// <param name="sourceDic">源字典</param>
        /// <param name="funcTo">类型转换回调</param>
        /// <returns>目标字典</returns>
        public static IDictionary<TargetKeyT, TargetValueT> DicToTypeDic<SourceKeyT, SourceValueT, TargetKeyT, TargetValueT>(this IDictionary<SourceKeyT, SourceValueT> sourceDic, Func<KeyValuePair<SourceKeyT, SourceValueT>, KeyValuePair<TargetKeyT, TargetValueT>> funcTo)
        {
            if (sourceDic.IsNullOrCount0())
            {
                return null;
            }
            if (funcTo == null)
            {
                throw new ArgumentNullException("类型转换回调不能为null");
            }

            var result = new Dictionary<TargetKeyT, TargetValueT>(sourceDic.Count);
            foreach (var item in sourceDic)
            {
                var newItem = funcTo(item);
