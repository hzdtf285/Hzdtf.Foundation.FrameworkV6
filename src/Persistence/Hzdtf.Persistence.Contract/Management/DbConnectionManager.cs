...
namespace Hzdtf.Persistence.Contract.Management
{
    /// <summary>
    /// 数据库连接管理器
    /// @ 黄振东
    ...
    public static partial class DbConnectionManager
    {
        ...
        /// 数据库连接字典
        /// </summary>
        private static readonly IDictionary<string, IList<DbConnectionEx>> dicDbConnections = new ConcurrentDictionary<string, IList<DbConnectionEx>>();
        ...
        /// 数据库事务字典
        /// </summary>
        private static readonly IDictionary<string, DbTransactionEx> dicDbTransaction = new ConcurrentDictionary<string, DbTransactionEx>();

        /// <summary>
        /// 当前数据库事务数
        /// </summary>
        public static int CurrDbTransactionCount
        {
            get => dicDbTransaction.Count;
        }
        ...
        public static void ReleaseAll()
        {
            ...
            if (dicDbTransaction.IsNullOrCount0())
            {
                return;
            }

            foreach (var itemTrans in dicDbTransaction)
            {
                try
                {
                    itemTrans.Value.DbTransaction.Dispose();
                }
                catch { }
            }
