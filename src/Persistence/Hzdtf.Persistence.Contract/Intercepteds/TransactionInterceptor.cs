...
using Hzdtf.Utility.Intercepteds;
using Hzdtf.Utility;
using Hzdtf.Persistence.Contract.Basic.Default;

namespace Hzdtf.Persistence.Contract.Intercepteds
{
    /// <summary>
    /// 事务拦截器
    ...
    public class TransactionInterceptor : AttrInterceptorBase<TransactionAttribute>
    {
        ...
        protected override void Intercept(BasicReturnInfo basicReturn, IInvocation invocation, TransactionAttribute attr, out bool isExecProceeded)
        {
            ...
            IPersistenceConnection perConn = null;
            perConn = App.GetServiceFromInstance<IPersistenceConnection>();
            if (perConn == null)
            {
                perConn = App.GetServiceFromInstance<IPersistenceConnectionDefault>();
                if (perConn == null)
                {
                    basicReturn.SetFailureMsg("未找到实现IPersistenceConnection或IPersistenceConnectionDefault接口的实现类");
                    return;
                }
            }
